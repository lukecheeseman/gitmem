...
#include "branching/eager_sync_protocol.hh"
#include "branching/lazy_sync_protocol.hh"
...
std::unique_ptr<SyncProtocol> make_protocol(SyncKind sync_kind) {
  switch (sync_kind) {
    ...
    case SyncKind::BranchingLazy:   return std::make_unique<branching::BranchingEagerSyncProtocol>();
  }
  std::unreachable();
}
